    ...
    return (
        <div className={"flex justify-center md:w-full md:gap-8 flex-col md:flex-row " + position}>
            <div className={" lg:w-full text-left md:w-1/2 flex flex-col justify-center"}>
                <h2 className={"text-3xl lg:text-4xl lg:text-5xl font-bold text-secondary-950 pb-4 border-b border-primary-600 mb-4"}>{title}</h2>
                {content.map((text, i) => (
                    <p key={i} className={"text-pretty text-xl md:text-xl lg:text-2xl text-secondary-950 pb-4"}>{text}</p>
                ))}
            </div>
            <div
