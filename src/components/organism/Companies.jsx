...
function Companies() {
    const brands = [
        ...
        {id: 3, name: "Maesrk", img: maesrkLogo},
        {id: 5, name: "Triton", img: tritonLogo},
        {id: 4, name: "Mediterranean shipping company", img: mediterraneanLogo, customClass: " md:p-0"},
    ]
    return (
        <SectionLayout id={"companies"} className={"bg-primary-600 gap-8 md:flex-col lg:flex-row"}>
            ...
            <div className={"flex flex-wrap justify-center gap-4 lg:grid lg:grid-cols-5 w-full"}>
                {brands.map(brand => {
                    return (
                        <div key={brand.id}
                             className={"w-[45%] lg:w-full h-[100px] flex justify-center items-center object-contain"}>
                            <img
